const waccaNews = [
  {
    title: "**[Notice]** Updates to News Feed, Homepage, and Gacha",
    date: "2026-09-11",
    body: `Hey everyone! Thank you for playing WACCA Plus! Your homepage might look a little different now...

We've added a news feed, so players can stay up to date on all the latest developments and updates.
This page should stay current with announcements posted in the [discord](https://discord.gg/hDZtQ2tVcW).

With these changes, a couple of things got moved around:

---

- *The Homepage is now your default landing page, and can be reached by clicking the big WACCA logo at the top of your screen*

- *The Gacha button has been removed, and is now its own tab (Gamba responsibly)*

- *You can get to the Cards page by clicking the icon to the right of your username at the top of the page*

- *Players can change their username from the Cards page by clicking the edit button next to their name*

---

Pretty cool right? There have also been some visual updates here and there around the leaderboards so check things out when you get the chance!

That's all for now. Let's Dive!

![Elizabeth](/wacca/img/news/eli.webp)`
  },
  {
    title: "「 The Devil Incarnate 2 」 Sightreading Tournament",
    date: "2026-09-10",
    body: `**『The Devil Incarnate // WACCA Stage-Up Sightreading Tournament 2』** is happening this weekend, boasting up to **30** Tournament exclusive charts!

Tune into the stream on September 12th, 2026 @ 4PM PST/7PM EST. You won't want to miss this!

https://twitch.tv/arcadelegacy
![TDI2](/wacca/img/news/tdi.webp)
`,
  },
  {
    title: "Touhou Update Wave 2",
    date: "2026-08-31",
    body: `# The Incident Continues...
\
Here comes wave **2** of the **2**hu charts, with ports from our friends at Reburn!

- *サドマミホリック | SadoMamiHolic*
- *つるぺったん | Tsurupettan*
- *Imperishable Night 2006 (2016 Refine)*
- *Fragmentary Specter Blade* **[INFERNO]**

And, some new and returning faces

- *Eden*
- *SUPER☆HARAGURO☆POP*
- *Jägermeister* **[INFERNO]**

![Touhou1](/wacca/img/news/touhou_1.webp)
![Touhou2](/wacca/img/news/touhou_2.webp)
![Touhou5](/wacca/img/news/touhou_5.webp)

---


We're also adding more charts from the SakuraCon 2026 tourney!

- *Avenue* **[INFERNO]**
- *N3V3R G3T OV3R* **[INFERNO]**
- *Strange Bar* **[INFERNO]**

![Sakura2026](/wacca/img/news/touhou_3.webp)

---

Last but not least, Lunatic mode has been unlocked

- *WARNING×WARNING×WARNING* **[INFERNO]**

![Touhou4](/wacca/img/news/touhou_4.webp)

Please look forward to it! Hopefully there won't be any strange incidents involving silent songs that m̴i̸g̶h̶t̴ ̷c̴a̴u̴s̴e̵ ̶a̸n̴y̵ ̷w̷e̸i̸r̴d̶ g̷̛̃̈́̃ľ̴̋̔͝i̸̊̓̅͠t̸c̷͂͑̄͝h̴͋͘͜è̶͗̉͘s̵̒̽̌͑.̶́̿͒̔.̷̑͂. C͔HECK THE R̶̤̣̽̀ECƠ̶̏̌̈Ȓ̴͝DS

###### **Some content may have to be unlocked first*
`
  },
];

export default waccaNews;
